...
func (mq *MessageQuery) loadFile(ctx context.Context, query *FileQuery, nodes []*Message, init func(*Message), assign func(*Message, *File)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Message)
	for i := range nodes {
...
func (mq *MessageQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := mq.ctx.Fields; len(fields) > 0 {
		...
		if mq.withFile != nil {
			_spec.Node.AddColumnOnce(message.FieldFileID)
		}
	}
