...
func (cq *ChatQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Chat, error) {
	...
	if query := cq.withImage; query != nil {
		if err := cq.loadImage(ctx, query, nodes, nil,
			func(n *Chat, e *File) { n.Edges.Image = e }); err != nil {
			return nil, err
		}
	}
...
func (cq *ChatQuery) loadImage(ctx context.Context, query *FileQuery, nodes []*Chat, init func(*Chat), assign func(*Chat, *File)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Chat)
	for i := range nodes {
...
func (cq *ChatQuery) loadUsers(ctx context.Context, query *UserQuery, nodes []*Chat, init func(*Chat), assign func(*Chat, *User)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[uuid.UUID]*Chat)
	nids := make(map[uuid.UUID]map[*Chat]struct{})
...
func (cq *ChatQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := cq.ctx.Fields; len(fields) > 0 {
		...
		if cq.withOwner != nil {
			_spec.Node.AddColumnOnce(chat.FieldOwnerID)
		}
		if cq.withImage != nil {
			_spec.Node.AddColumnOnce(chat.FieldImageID)
		}
	}
