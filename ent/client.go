...
func (c *Client) Use(hooks ...Hook) {
	for _, n := range []interface{ Use(...Hook) }{
		c.Chat, c.City, c.Comment, c.Country, c.File, c.Friend, c.Message,
		c.Notification, c.Otp, c.Post, c.Reaction, c.Region, c.Reply, c.SiteDetail,
		c.User,
	} {
		n.Use(hooks...)
	}
...
func (c *ChatClient) QueryImage(ch *Chat) *FileQuery {
	...
	query.path = func(context.Context) (fromV *sql.Selector, _ error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.M2O, true, chat.ImageTable, chat.ImageColumn),
		)
		fromV = sqlgraph.Neighbors(ch.driver.Dialect(), step)
		return fromV, nil
	}
...
func (c *FileClient) QueryChats(f *File) *ChatQuery {
	...
	query.path = func(context.Context) (fromV *sql.Selector, _ error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.O2M, false, file.ChatsTable, file.ChatsColumn),
		)
		fromV = sqlgraph.Neighbors(f.driver.Dialect(), step)
		return fromV, nil
	}
...
func (c *FileClient) QueryMessages(f *File) *MessageQuery {
	...
	query.path = func(context.Context) (fromV *sql.Selector, _ error) {
		...
		step := sqlgraph.NewStep(
			sqlgraph.From(file.Table, file.FieldID, id),
			sqlgraph.To(message.Table, message.FieldID),
			sqlgraph.Edge(sqlgraph.O2M, false, file.MessagesTable, file.MessagesColumn),
		)
		fromV = sqlgraph.Neighbors(f.driver.Dialect(), step)
		return fromV, nil
	}
...
func (c *MessageClient) QueryFile(m *Message) *FileQuery {
	...
	query.path = func(context.Context) (fromV *sql.Selector, _ error) {
		...
		step := sqlgraph.NewStep(
			sqlgraph.From(message.Table, message.FieldID, id),
			sqlgraph.To(file.Table, file.FieldID),
			sqlgraph.Edge(sqlgraph.M2O, true, message.FileTable, message.FileColumn),
		)
		fromV = sqlgraph.Neighbors(m.driver.Dialect(), step)
		return fromV, nil
	}
...
type (
	hooks struct {
		Chat, City, Comment, Country, File, Friend, Message, Notification, Otp, Post,
		Reaction, Region, Reply, SiteDetail, User []ent.Hook
	}
	inters struct {
		Chat, City, Comment, Country, File, Friend, Message, Notification, Otp, Post,
		Reaction, Region, Reply, SiteDetail, User []ent.Interceptor
	}
)
